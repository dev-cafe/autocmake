...
/* MKL, OpenBLAS   */
  ...
  if (abs(b[0] + 0.500) <= small && abs(b[1] - 0.250) <= small && abs(b[2] - 0.250) <= small) roots_ok=1;
  if ( roots_ok == 1 ) {printf("PASSED"); exit(EXIT_SUCCESS);}
  else {printf("\nC LAPACK dgesv failure!");exit(EXIT_FAILURE);}
}

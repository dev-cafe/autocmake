...
def autogenerated_notice():
    ...
    s = []
    s.append(
        "# This file is autogenerated by Autocmake v{0} http://autocmake.org".format(
            __version__
        )
    )
    s.append(
        "# Copyright (c) {0} by Radovan Bast, Roberto Di Remigio, Jonas Juselius, and contributors.".format(
            year_range
        )
    )
...
def gen_setup(config, default_build_type, relative_path, setup_script_name):
    ...
    options.append(
        [
            "--generator=<STRING>",
            "Set the CMake build system generator [default: Unix Makefiles].",
        ]
    )
    ...
    options.append(
        [
            ...
            "Set the CMake executable [default: cmake].",
        ]
    )
    options.append(
...
def gen_cmakelists(
    ...
    s.append(
        "cmake_minimum_required(VERSION {0} FATAL_ERROR)".format(min_cmake_version)
    )
    ...
    for module in modules:
        s.append(
            "include({0})".format(
                os.path.join(
                    "${PROJECT_SOURCE_DIR}", relative_path, module.path, module.name
                )
            )
        )
